...
import { LayoutDashboard, Activity, User, Github, X, Menu, Zap, Bot, BarChart2 } from 'lucide-react';
...
const links = [
  ...
  { href: ROUTES.CHAT,      label: 'AI Chat',   icon: Bot,              desc: 'Ask AI assistant'    },
  { href: ROUTES.COMPARE,   label: 'Compare',   icon: BarChart2,        desc: 'vs. avg developer'   },
  { href: ROUTES.PROFILE,   label: 'Profile',   icon: User,             desc: 'Account & history'   },
];
  ...
  <div className="flex flex-col h-full">
        ...
        return (
          <Link key={href} href={href} onClick={onNav}
            className={cn(
              ...
              active ? 'nav-active text-brand' : 'text-text-secondary hover:text-text-primary hover:bg-bg-tertiary/60'
            )}>
            ...
            <div className="min-w-0">
              ...
              <p className={cn('text-[10px] mt-0.5 truncate', active ? 'text-brand/60' : 'text-text-muted')}>{desc}</p>
            </div>
            {active && (
              <motion.div layoutId="sidebar-active" className="absolute right-2 h-1.5 w-1.5 rounded-full bg-brand"
    ...
    <div className="px-3 py-4 border-t border-border">
      <div className="flex items-center gap-2 px-3 py-2.5 rounded-lg bg-brand/5 border border-brand/10">
        ...
        <p className="text-[11px] text-text-secondary leading-snug">Analyze any GitHub profile instantly</p>
      </div>
    </div>
  </div>
);
        ...
        {mobileOpen && (
          <>
            ...
            <motion.aside
              ...
              <button className="absolute top-4 right-4 p-1.5 rounded-lg text-text-muted hover:text-text-primary hover:bg-bg-tertiary transition-colors"
